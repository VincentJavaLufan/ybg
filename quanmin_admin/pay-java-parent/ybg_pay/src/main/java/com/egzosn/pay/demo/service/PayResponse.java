...
public class PayResponse {
	...
	private void buildRouter(Integer payId) {
		...
		router.rule()
				...
				// 处理器
				.handler(autowire(new AliPayMessageHandler(payId))).end().rule().msgType(MsgType.xml.name()).payType(PayType.wxPay.name()).handler(autowire(new WxPayMessageHandler(payId))).end().rule().msgType(MsgType.json.name()).payType(PayType.youdianPay.name()).interceptor(new YoudianPayMessageInterceptor()) // 拦截器
